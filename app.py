...
st.title("Une maison en Bretagne")
st.header("Resultats")
st.markdown("---")
...
trier_par = st.sidebar.selectbox(
    "Trier par",
    [
        "Date",
        "Score",
        "Prix décroissant",
        "Prix croissant",
        "Taille de maison décroissante",
        "Taille de terrain décroissante",
    ],
)
...
def treguet():
    ...
    for npage in range(1, num_pg + 1):
        ...
        URL = (
            "http://www.cabinet-treguer.fr/advanced-search/page/%s/?bedrooms&keyword=29160&max-area&max-price&min-area&status=vente&type=maison"
            % str(npage)
        )
        page = requests.get(URL)
        soup = BeautifulSoup(page.content, "html.parser")
        ...
        for val in results:
            ...
            if ("vue" in vue_mer or "bord" in vue_mer or "accès" in vue_mer) and (
                "mer" in vue_mer or "grève" in vue_mer or "baie" in vue_mer
            ):
                to_ret = "Oui"
...
def bourse_immo():
    ...
    for l in soup.find_all(class_="bottom"):
        ...
        if "annonce" in lk:
            ...
            vue_mer = "Non"
            if ("vue" in descr or "bord" in descr or "accès" in descr) and (
                "mer" in descr or "grève" in descr or "baie" in descr
            ):
                vue_mer = "Oui"
...
def ouest_france():
    ...
    for val in results.find_all(class_="annLink", href=True):
        ...
        for pic in photos:
            if pic != "\n":
                list_photos.append(
                    str(pic).split("src=")[1].split(" title=")[0].replace('"', "")
                )
...
def agence_presquile():
    ...
    for i in range(1, int(results[1].text) + 1):
        ...
        URL = (
            "https://www.agencedelapresquile.com/achat-immobilier-crozon/type-1-maison/"
            + str(i)
        )
        page = requests.get(URL)
...
if st.sidebar.button("Actualiser"):
    ...
    df_concat["score"] = df_concat.apply(
        lambda x: compute_score(
            x["Surface"],
            max(df_concat["Surface"]),
            x["Terrain"],
            max(df_concat["Terrain"]),
            x["VueMer"],
        ),
        axis=1,
    )

    df_concat["Remboursement"] = df_concat["Prix"].apply(
        lambda x: compute_remboursement(x)
    )
...
for val in df_concat.iterrows():
    if val[1]["VueMer"] == "Oui":
        st.subheader(
            "Maison vue Mer à %s de %s m² sur terrain de %s m²"
            % (val[1]["Ville"], val[1]["Surface"], val[1]["Terrain"])
        )
    else:
        st.subheader(
            "Maison à %s de %s m² sur terrain de %s m²"
            % (val[1]["Ville"], val[1]["Surface"], val[1]["Terrain"])
        )
    st.write(" ")
    st.write(
        ...
        val[1]["Prix"],
        " €, soit ",
        np.round(int(val[1]["Remboursement"])),
        " €/mois",
    )
    ...
    if st.checkbox("Voir photos", key=val):
        for p in val[1]["Photos"][1:-1].split(","):
            
            st.image(p.replace("'", "").strip(), width=500)
